...
def create_images_dir(dir='./images'):
    global images_dir
    images_dir = dir
...
def capture_image():
    ...
    time.sleep(5) # Give the camera time to initialize
...
def create_gif(output_file):
    ...
    for filename in os.listdir(images_dir):
        if filename.endswith(".jpg"):
            ...
            images.append(image)
            #image.close() # Close image after appending to save memory
...
def main():
    ...
    monitor_thread = None
    start_date = datetime.now().strftime("%Y-%m-%d") # Date when timelapse was started
    ...
    try:
        ...
        gif_filename = f"./timelapse_{start_date}_{end_date}.gif"
